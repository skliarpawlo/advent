(ns advent.tasks-2020.task-2)

(defn task-2 [policies]
  (->> policies
       (map parse-policy)
       (map is-valid-v2)
       (filter true?)
...
(defn is-valid-v2 [{min-val :min
                    ...
                    letter :letter
                    password :password}]
  (not= (= letter (nth password (dec min-val)))
        (= letter (nth password (dec max-val)))))


(defn is-valid [{min-val :min
                 max-val :max
                 letter :letter
                 password :password}]
(let [letter-count (count-char password letter)]
  (and (<= min-val letter-count)
       (>= max-val letter-count))))


(defn parse-policy [policy]
(let [[_ min-val max-val letter password]
      (re-matches #"(\d+)-(\d+) ([a-z]): ([a-z]*)" policy)
      letter-char (-> letter .toCharArray first)]
  {:min (Integer. min-val)
   :max (Integer. max-val)
   :letter letter-char
   :password password}))
